...
# adicionei a função chapeu para o caso mais simples
...
def chapeu(n,x0,xn):
    ...
    phi_i = "(x-"+str(x0)+")/"+str(h)
    phi_f = "("+str(xn) +" - x)/"+str(h)

    return np.array([phi_i,phi_f])
...
def fem(n):
    ...
    # adicionei esse loop que calcula a matriz dos phis

    for i in range (1,n+1):

        phi = chapeu(n,x[i+1],x[i-1])
        
        phis = np.array(eval(phi[0],{'x':x[i]}))
