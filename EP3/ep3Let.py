...
def q(Qp0, Qm0, sigma, theta, L, x):
    q = Qp0*np.exp(-((x-L)/sigma)**2) - Qm0*(np.exp(-(x/theta)**2) + np.exp(-((x-L)/theta)**2))
    return q
...
def mat_varia(q,L,n,d):
  
  k1=3.6
  k2=60

  material1= solve_generic(n,L,k1,q,L/2-d,L/2+d)
  material2 = solve_generic(n,L,k2,q,0,L/2-d) + solve_generic(n,L,k2,q,L/2+d,L)

  return material1+material2

def grafico_forçantes(forc,):
